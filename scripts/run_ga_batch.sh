#!/usr/bin/env bash
# Run H_* GA instances sequentially using detached screen sessions.
# Each instance is launched only after the previous screen session exits.
#
# NOTE: GA has not yet been through irace tuning.
# Parameters are working defaults: pop_size=400, clone_threshold=0.1
#
# IMPORTANT (remote server): run this script itself inside a persistent session
# so that it survives SSH disconnects, e.g.:
#
#   screen -S ga_batch -dmL -Logfile logs_ga_beta[3/6]/ga_batch.log \
#       bash scripts/run_ga_batch.sh
#
# or:
#   nohup bash scripts/run_ga_batch.sh > logs_ga_beta[3/6]/ga_batch.log 2>&1 &
#
# Usage:
#   bash scripts/run_ga_batch.sh [instances_file] [--beta VALUE]
#   bash scripts/run_ga_batch.sh scripts/list_ga_all.txt --beta 12
...
INSTANCES_FILE="${INSTANCES_FILE:-"$SCRIPT_DIR/list_ga_all.txt"}"

LOG_DIR="$PROJECT_DIR/logs_ga_beta3"
...
while IFS= read -r instance || [[ -n "$instance" ]]; do
    ...
    screen -S "$session" -dmL -Logfile "$logfile" \
        ...
        "$SCRIPT_DIR/run_ga_cli.jl" "--instance=$instance" ${BETA_ARG:+$BETA_ARG}
