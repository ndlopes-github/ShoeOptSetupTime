...
function parse_arguments()
    ...
    for arg in ARGS
        ...
        elseif arg == "--help" || arg == "-h"
            ...
            println("  --skip-ga          Skip Genetic Algorithm (mark as Skipped in output)")
...
const DRY_RUN_LIMIT, SKIP_METHODS, BETA_OVERRIDE, ONLY_FILE = parse_arguments()
...
"""
Run GA multiple times and return best solution info
"""
function run_ga_multiple(order_dict::Dict, num_runs::Int)
    @unpack α, β = order_dict
    ...
    try
        ...
        for run_idx in 1:num_runs
            run_time = @elapsed begin
                shelves = GeneticAlgorithm.run_ga(order_dict;
        ...
        instance_id = "$(order_dict[:Oid])"
        @warn "GA failed for $(instance_id): $e"
        log_error(ERROR_LOG, "Genetic Algorithm", instance_id, e, catch_backtrace())
        return (cost="Err", m="Err", time="Err", error=true)
    end
end
...
println("  GA runs per instance: $(NUM_GA_RUNS)  pop_size=$(GA_POP_SIZE)  clone_threshold=$(GA_CLONE_THRESHOLD)")
...
processed_count = 0
method_counts = Dict("MILP" => 0, "SSM" => 0, "SA" => 0, "GRASP" => 0, "GA" => 0)
    ...
    try
        ...
        if haskey(files, "E")
            ...
            if @isdefined(order_dict)
                ...
                # Run GA with individual error handling
                if "GA" in SKIP_METHODS
                    println("  Skipping GA...")
                    ga_result = (cost="Skipped", m="Skipped", time="Skipped", error=false)
                else
                    println("  Running GA ($(NUM_GA_RUNS) runs)...")
                    try
                        ga_result = run_ga_multiple(order_dict, NUM_GA_RUNS)
                        if ga_result.error
                            println("    GA: ❌ ERROR")
                        else
                            println("    GA: cost=$(ga_result.cost), m=$(ga_result.m), time=$(round(ga_result.time, digits=3))s")
                        end
                    catch e
                        instance_id = "$(order)_#$(scenario)_$(p)p"
                        @warn "Unexpected error running GA: $e"
                        log_error(ERROR_LOG, "GA (outer catch)", instance_id, e, catch_backtrace())
                        ga_result = (cost="Err", m="Err", time="Err", error=true)
                        println("    GA: ❌ ERROR")
                    end
                end
            end
        ...
        row = (
            ...
            GA_Cost=ga_result !== nothing ? ga_result.cost : missing,
            GA_Slots=ga_result !== nothing ? ga_result.m : missing,
            GA_Time=ga_result !== nothing ? ga_result.time : missing
        )
        ...
        !ismissing(row.GRASP_Cost) && row.GRASP_Cost != "Skipped" && (method_counts["GRASP"] += 1)
        !ismissing(row.GA_Cost) && row.GA_Cost != "Skipped" && (method_counts["GA"] += 1)        # Mark instance as completed in progress log
...
println(@sprintf("Instances with GA results: %d", method_counts["GA"]))
