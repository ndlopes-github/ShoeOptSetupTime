#!/usr/bin/env bash
# Run all 60 H_* pure SA instances sequentially using detached screen sessions.
# Each instance is launched only after the previous screen session exits.
#
# Parameters are set to the irace best configuration:
#  for beta=3 -> T0=0.7724  Tf=0.1615  Nit=1885  Tj=11  Nruns=196
#  for beta=6 -> T0=48.2242 Tf=0.0791  Nit=1995  Tj=13  Nruns=181

# IMPORTANT (remote server): run this script itself inside a persistent session
# so that it survives SSH disconnects, e.g.:
#
#   screen -S sa_batch -dmL -Logfile logs_sa_beta?/sa_batch.log \
#       bash scripts/run_sa_batch.sh
#
# or:
#   nohup bash scripts/run_sa_batch.sh > logs_sa_beta?/sa_batch.log 2>&1 &
#
# Usage:
#   bash scripts/run_sa_batch.sh [instances_file]
#   bash scripts/run_sa_batch.sh scripts/list_sa_all.txt
...
INSTANCES_FILE="${INSTANCES_FILE:-"$SCRIPT_DIR/list_sa_all.txt"}"

LOG_DIR="$PROJECT_DIR/logs_sa_beta6"
POLL_INTERVAL=60   # seconds between screen-session checks

mkdir -p "$LOG_DIR"

echo "=== Pure SA batch run ==="
echo "Instances file  : $INSTANCES_FILE"
echo "Log directory   : $LOG_DIR"
echo "Project dir     : $PROJECT_DIR"
echo "Poll interval   : ${POLL_INTERVAL}s"
echo "Parameters      : T0=0.7724  Tf=0.1615  Nit=1885  Tj=11  Nruns=196  (irace best config for beta = 3)"
echo "Parameters      : T0=48.2242 Tf=0.0791  Nit=1995  Tj=13  Nruns=181  (irace best config for beta = 6)"
...
while IFS= read -r instance || [[ -n "$instance" ]]; do
    ...
    screen -S "$session" -dmL -Logfile "$logfile" \
        ...
        "$SCRIPT_DIR/run_sa_cli.jl" "--instance=$instance" ${BETA_ARG:+$BETA_ARG}
