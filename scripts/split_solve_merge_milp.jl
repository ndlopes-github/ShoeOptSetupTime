...
function model_builder(instance; x_cnst_indxs=[], order_dict)
    ...
    if solver_name == "Gurobi"
        ...
        set_optimizer_attribute(model, "Threads", 60) #60
        ...
        set_optimizer_attribute(model, "SoftMemLimit", 128)  #128 In GB
...
function solver(model, instance; order_dict)
    ...
    if !is_solved_and_feasible(model; dual=false)
        if !has_values(model)
            @warn """
              Model Unsolved: No feasible solution found.
              termination_status : $(termination_status(model))
              primal_status      : $(primal_status(model))
              dual_status        : $(dual_status(model))
              raw_status         : $(raw_status(model))
              Returning penalty cost for irace to skip this configuration.
              """
            # Return penalty cost so irace can skip this configuration
            penalty_cost = 1e6
            status = termination_status(model)
            table_dict = @strdict α β p m=0 cost=penalty_cost cost_p=0 bound=penalty_cost gap=0 stime=0 MiB status
            sol_dict = @dict Dxf=Dict() Dtf=Dict() Idxs=[]
    ...
    gap = abs(cost) > 0 ? abs(cost - bound) / abs(cost) : 0.0

    if termination_status(model) == MOI.TIME_LIMIT
        @warn @sprintf("Time limit reached: feasible=%.4f, bound=%.4f, gap=%.2f%%", cost, bound, 100 * gap)
        @info @sprintf("Time limit reached: feasible=%.4f, bound=%.4f, gap=%.2f%%", cost, bound, 100 * gap)
    end
    ...
    status = termination_status(model)
    table_dict = @strdict α β p m cost cost_p bound gap stime MiB status
...
function process_instances_group(instances_group; order_dict)
    ...
    for i in eachindex(instances_group)
        df = DataFrame(:α => [], :β => [], :p => [], :m => [], :cost => [], :cost_p => [], :bound => [], :gap => [], :stime => [], :MiB => [], :status => [])
        sols_dict = []
...
function run(order_dict)
    ...
    with_logger(demux_logger(prefix)) do
        ...
        elapsed_time += @elapsed begin
            ...
            @info "Sub-solve bound and gap per iteration available in logs above"
        end
        @info "Elapsed time $(elapsed_time) seconds"
