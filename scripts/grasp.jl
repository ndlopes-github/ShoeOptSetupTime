...
"""
function run(order_dict; log_every::Int=10, selection::Symbol=:probabilistic, split_method::Symbol=:random, iterations::Union{Nothing,Int}=nothing, return_costs::Bool=false)
    ...
    for iteration in 1:iters
        ...
        push!(run_costs, current_cost)
        push!(run_ms, current_m)
        push!(run_times, iter_time)
    ...
    @info dfp
    return return_costs ? (shelves=best_shelves, run_costs=run_costs, run_ms=run_ms, run_times=run_times) : best_shelves
end
